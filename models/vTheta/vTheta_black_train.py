...
def training(train_size, test_size, cv_size, training_batch_size, training_rate):
	...
	if tf.gfile.Exists('tmp/vTheta_black'):
		tf.gfile.DeleteRecursively('tmp/vTheta_black')
	tf.gfile.MakeDirs('tmp/vTheta_black')

	# placehold and input
	with tf.name_scope('input'):
		x = tf.placeholder(tf.float32, [None, 15, 15, 5], 'vinput_layer')
		y_ = tf.placeholder(tf.float32, [None, 1], 'vy_label')
	...
	y = nn_fc_layer(hidden_conv_layer7, 15 * 15, 1, 'voutput_layer', tf.tanh)

	# results
	with tf.name_scope('results'):
		loss = tf.reduce_mean(-tf.reduce_mean((y_ - y) ** 2), reduction_indices = 1))
	...
	for k in range(10):
		...
		with tf.Session() as sess:
			train_writer = tf.train.SummaryWriter('tmp/vTheta_black/train', sess.graph)
			test_writer = tf.train.SummaryWriter('tmp/vTheta_black/test', sess.graph)

			sess.run(init)
			# if (order == 'y'):
			if (k > 0):
				saver.restore(sess, 'tmp/vTheta_black.ckpt')
