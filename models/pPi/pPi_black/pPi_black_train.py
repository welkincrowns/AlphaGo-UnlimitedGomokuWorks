...
def training(train_size, test_size, cv_size, training_batch_size, training_rate):
	...
	if tf.gfile.Exists('tmp/pPi_black'):
		tf.gfile.DeleteRecursively('tmp/pPi_black')
	tf.gfile.MakeDirs('tmp/pPi_black')
	...
	for k in range(10):
		...
		with tf.Session() as sess:
			train_writer = tf.train.SummaryWriter('tmp/pPi_black/train', sess.graph)
			test_writer = tf.train.SummaryWriter('tmp/pPi_black/test', sess.graph)
			sess.run(init)
			# if (order == 'y'):
			if (k > 0):
				saver.restore(sess, 'tmp/pPi_black.ckpt')
			...
			for i in range(20001):
				if (i % 100 == 0):
					...
					if (sacc > max_acc):
						max_acc = acc
					save_path = saver.save(sess, 'tmp/pPi_black.ckpt')
